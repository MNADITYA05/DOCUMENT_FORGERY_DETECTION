...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, x1, x2):
        x1_seq, x2_seq = x1.unsqueeze(1), x2.unsqueeze(1)
        
        cross_out, _ = self.cross_attn(x1_seq, x2_seq, x2_seq)
        x1_seq = self.norm1(x1_seq + self.dropout(cross_out))
        
        self_out, _ = self.self_attn(x1_seq, x1_seq, x1_seq)
        x1_seq = self.norm2(x1_seq + self.dropout(self_out))
        
        ffn_out = self.ffn(x1_seq)
        x1_seq = self.norm3(x1_seq + self.dropout(ffn_out))
...
class EnhancedDocumentForgeryDetector(nn.Module):
    def __init__(self, num_classes=1, feature_dim=768, nhead=12, dropout=0.1):
        ...
        self.visual_to_fft = CrossModalAttention(feature_dim, nhead, dropout)
        self.fft_to_visual = CrossModalAttention(feature_dim, nhead, dropout)
    ...
    def forward(self, x):
        ...
        v2f_feat = self.visual_to_fft(visual_feat, fft_feat)
        f2v_feat = self.fft_to_visual(fft_feat, visual_feat)
        
        gate_weight = self.gate(torch.cat([v2f_feat, f2v_feat], dim=1))
        fused_feat = gate_weight * v2f_feat + (1 - gate_weight) * f2v_feat
        
        ensemble_outputs = []
        for head in self.ensemble_heads:
            ensemble_outputs.append(head(fused_feat))
        
        ensemble_probs = [torch.sigmoid(out) for out in ensemble_outputs]
        ensemble_tensor = torch.cat(ensemble_probs, dim=1)
        
        meta_input = torch.cat([fused_feat, ensemble_tensor], dim=1)
...
CONFIG = {
    ...
    "MODEL_SAVE_PATH": "enhanced_model.pth",
    "PLOT_SAVE_PATH": "enhanced_metrics.png",
...
def visualize_test_samples(model, test_dataloader, device, num_samples=16):
    ...
    plt.tight_layout()
    plt.savefig('enhanced_test_samples.png', dpi=150, bbox_inches='tight')
    plt.show()
